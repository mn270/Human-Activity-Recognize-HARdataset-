...
def load_dataset(prefix='/home/marcin/Pobrane/'):
    ...
    testy = testy - 1
    # SHOW DATA
    # big_data = np.concatenate((trainy,testy))
    # LABELS = ["Chód", "Chód w górę", "Chód w dół", "Siedzenie", "Stanie", "Leżenie"]
    # _, counts_elements = np.unique(big_data, return_counts=True)
    # plt.bar(LABELS, counts_elements)
    # plt.title('Częstotliwość występowania czynności w zbiorze')
...
#    trainX, testX = Power_Spectrum(trainX,testX)
    ...
    return trainX, trainy, testX, testy, testy_norm
